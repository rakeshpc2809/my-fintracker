...
class FifoMatcherTest {
    ...
    fun `test fifo matching on acquisitions and disposals`() {
        ...
        assertEquals(BigDecimal("100.00"), slice1.unitsMatched)
        assertEquals(BigDecimal("1000.00"), slice1.costBasis)
        assertEquals(BigDecimal("1500.00"), slice1.saleProceeds)
        assertEquals(BigDecimal("500.00"), slice1.realizedGain)

        // Second slice: 20 units from lot 2 (acquired 2024-06-10, disposed 2025-02-15 => <365 days => Short term)
        val slice2 = matchedLots[1]
        assertEquals(BigDecimal("20.00"), slice2.unitsMatched)
        assertEquals(BigDecimal("240.00"), slice2.costBasis)
        assertEquals(BigDecimal("300.00"), slice2.saleProceeds)
        assertEquals(BigDecimal("60.00"), slice2.realizedGain)
    }
